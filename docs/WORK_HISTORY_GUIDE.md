...
1. **이슈 생성**: 작업 시작 전 GitHub 이슈 생성 (제목, 작업 내용, 체크리스트 포함)
2. **작업 진행**: 코드 작성 및 테스트
3. **HISTORY.md 업데이트**: 이슈 단위로 작업 내용 기록
4. **커밋**: 의미있는 커밋 메시지와 함께 커밋
5. **이슈 종료**: 작업 완료 후 이슈에 완료 코멘트 남기고 종료
...
✅ 올바른 예:
1. gh issue create (이슈 #10 생성)
2. 코드 작성
3. HISTORY.md에 작업 내용 기록
4. git commit (커밋 메시지에 이슈 언급)
5. gh issue close #10 --comment "완료"
```
...
6. **소요 시간**: 실제 작업 시간 기록
7. **회고 작성**: 매일, 매주 회고 작성
8. **이슈 단위 기록**: 각 이슈별로 HISTORY.md 엔트리 작성
